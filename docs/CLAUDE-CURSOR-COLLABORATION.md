...
本文档为 **docs/** 下关于 Claude Code 与 Cursor 协作的权威说明。两工具通过 Graphiti 与 docs/ 共享上下文，分工明确、委托可追溯。**任务开始/结束的完整开发流程**（任务开始：Graphiti 必做 → GitNexus 代码类 → 读文档不足时 → issue+分支 开发/修复/功能类 → SubAgent；改代码前 impact；任务结束：detect_changes → commit/push → Graphiti 记录 → 更新文档/配置 → PR）以本文件与 `.cursor/rules/task-priority-workflow.mdc` 为准。
...
| **GitNexus** | 代码结构索引（MCP 共享）；主 Agent 委托 **gitnexus** SubAgent：任务开始 detect_changes + query，改代码前 impact，commit 前 detect_changes |
| **docs/** | 唯一文档根，架构、迁移、规范等均在此；两工具共同维护 |
| **Skills** | `.agents/skills/` 为共享技能库：Claude Code 经 `.agent/skills/`、Cursor 经 `.cursor/skills/` 访问 |

## Claude Code 侧：MEMORY.md（最可靠来源）

Claude Code 会**自动生成**项目记忆文件 **MEMORY.md**，存放在本机 **`~/.claude/projects/<project>/memory/MEMORY.md`**（按项目路径编码，不在仓库内）。Claude Code 会把在该项目中发现的构建命令、调试经验、API 约定、偏好等写入该文件；**前 200 行会在每次会话加载**，因此 Claude Code 侧视其为**最可靠的项目记忆来源**。

- **CLAUDE.md**：你写的入口与规则；**MEMORY.md**：Claude 自动写的项目学习与模式。
- Cursor 无法直接读取 `~/.claude/...` 下的 MEMORY.md。若要与 Cursor 对齐工作流或共享「最可靠」记忆，可：
  - 将 MEMORY.md 中与协作/流程相关的段落**定期复制到仓库**（例如 `docs/MEMORY-CLAUDE-CODE.md` 或 `.claude/MEMORY.md`），或
  - 在协作文档中注明「Claude Code 侧以 MEMORY.md 为准，Cursor 侧以 .cursor/rules 与 Graphiti 为准；若需一致，可把 MEMORY.md 关键内容同步到 docs/」。
...
| **MEMORY.md** | Claude Code 自动生成（`~/.claude/projects/.../memory/MEMORY.md`），Claude 侧最可靠的项目记忆；可选同步到仓库供 Cursor 参考 |
...
MCP 所需密钥与参数统一放在**项目根目录**的 `.env` 中；通过**同步脚本**把 `.env` 写入 `.cursor/mcp.json`，之后**直接打开 Cursor 即可，无需每次手动加载 .env**。

| 步骤 | 说明 |
|------|------|
| 1. 模板与配置 | 根目录 **`.env.example`** 列出 MCP 相关变量；复制为 **`.env`** 并填写实际值（`.env` 已 gitignore）。 |
| 2. 同步 | **修改 .env 后**在项目根执行一次 **`./sync-mcp-from-env.sh`**，会从 `.env` 生成 `.cursor/mcp.json`。之后像平时一样打开 Cursor，MCP 即可用。 |

- 模板文件：**.cursor/mcp.json.template**（可提交）；生成的 **.cursor/mcp.json** 含密钥，已 gitignore。
- 应用运行时环境变量（Next.js、Supabase、Docker 等）见 **Web/.env.example**，与根目录 MCP 用 `.env` 分离。
