...
任务结束完整步骤（detect_changes → commit/push → Graphiti → 文档/配置 → PR）以 `task-priority-workflow.mdc` 与 `docs/CLAUDE-CURSOR-COLLABORATION.md` 为准。执行 Rule `post-task-workflow.mdc` 和 Skill `post-task-workflow` 定义的两条流程：
