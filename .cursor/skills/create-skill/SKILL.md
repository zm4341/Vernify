...
在 Vernify 项目中创建 Skill 时，须遵循：

1. **先查询是否已有类似**：在 .cursor/skills/、.cursor/rules/、.cursor/agents/、.cursor/commands/ 中查询是否已有相同或可复用的配置
2. **若无则构建完整配套**：新建 Skill 时考虑是否需配套 Rule + SubAgent + Command，使能力可被发现、可被委托、可被触发
3. **原子化、细粒度可组合**：Skill 可独立使用、与 Rules/SubAgents 配合；尽量细粒度可组合，避免大而全的单体
4. **参考**：`project-maintenance.mdc` 中 Rules/Skills/SubAgents/Commands 创建约定
